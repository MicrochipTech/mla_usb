...
// *****************************************************************************
        ...
        // barcodes).  This is the value of the first data uint8_t of the bar code
        // data, which begins the character code specification.  The next uint8_t
        // must be 'A', 'B', or 'C'.
...
#define BARCODE_CODE128_CODESET_STRING          "{"
        // For use with POS printers that support Code128 bar codes (extended
        // barcodes).  This is the value of the second data uint8_t of the bar code
...
#define EVENT_PRINTER_DETACH        (EVENT_PRINTER_BASE+EVENT_PRINTER_OFFSET+1)
        ...
        // actual number of uint8_ts that were written to the device.
#define EVENT_PRINTER_TX_DONE       (EVENT_PRINTER_BASE+EVENT_PRINTER_OFFSET+2)
        ...
        // When USB_HOST_APP_EVENT_HANDLER is called with this event, *data
        // points to the receive buffer, and size is the actual number of uint8_ts
...
typedef enum
{
    ...
    // point to the data to be sent, and size should indicate the number of uint8_ts
    ...
    // to the buffer of data to send, and size should indicate how many uint8_ts
    ...
    // passed in the most significant (upper) uint16_t, and the Y coordinate is
    // passed in the least significant (lower) uint16_t.  The macro
    ...
    // passed in the most significant uint16_t of the size parameter, and the Y-axis
    // position value is passed in the least significant uint16_t of the size
    ...
    // position change is passed in the most significant uint16_t of the size
    // parameter, and the Y-axis position change is passed in the least
    // significant uint16_t of the size parameter.  POS printers do not support
    ...
    // uint16_t of the size parameter, and the Y-axis position value is passed in
    // the least significant uint16_t of the size parameter.  The data pointer
    // parameter is not used and should be set to USB_NULL.
    ...
    // is passed in the most significant uint16_t of the size parameter, and the
    // Y-axis displacement is passed in the least significant uint16_t of the size
    ...
    // significant uint8_t of the size parameter indicates the number of vertical
    ...
    // The least significant uint8_t of the size parameter indicates the number of
...
typedef enum
{
    ...
    USB_PRINTER_POS_BARCODE_UPC_A = 0,
        ...
        // lengths of 6, 7, or 11 uint8_ts are supported.  Not all printers support
        // the 6 or 7 uint8_t widths; 11 uint8_t data is recommended.  If the data length is
        // not 6, then the first the first digit (the number system character)
        // must be set to '0'.  If 11 data uint8_ts are presented, the printer will
    ...
    USB_PRINTER_POS_BARCODE_CODE93,
        ...
        // 128-bit ASCII plus special symbols.  The first two data uint8_ts must
        // be the code set selection character.  The first uint8_t must be
        // BARCODE_CODE128_CODESET (0x7B), and the second uint8_t must be 'A',
        ...
        // be encoded as two uint8_ts with the value 0x7B.
...
typedef union
{
    uint16_t    val;                    // The uint16_t representation of the support flags.
    struct
    {
        uint16_t    supportsVectorGraphics  : 1;    // The printer supports vector graphics.
        uint16_t    supportsPOS             : 1;    // The printer is a POS printer.
...
typedef struct
{
    ...
    uint16_t    positionX;          // The position of the image on the X axis.
    uint16_t    positionY;          // The position of the image on the Y axis.
    union
    {
        struct
        {
            uint16_t    resolution;         // (Full sheet printers only.) The resolution
        ...
        struct
        {
            uint8_t    densityVertical;    // (POS printers only.) The vertical dot
                                        // density of the bit image.  Valid values
                                        // are printer dependent.  See above.
            uint8_t    densityHorizontal;  // (POS printers only.)  The horizontal dot
...
typedef union
{
    ...
    struct
    {
        uint16_t    xL;     // X-axis position of the upper left corner.
        uint16_t    yT;     // Y-axis position of the upper left corner.
        uint16_t    xR;     // X-axis position of the lower right corner.
        uint16_t    yB;     // Y-axis position of the lower right corner.
        uint16_t    r1;     // The inner radius of the two concentric cicles that defines the thickness of the arc.
        uint16_t    r2;     // The outer of radius the two concentric circles that defines the thickness of the arc.
        uint16_t    octant; // Bitmask of the octant that will be drawn.  Moving in a clockwise direction from x = 0, y = +radius
                        //     * bit0 : first octant
                        //     * bit1 : second octant
                        //     * bit2 : third octant
                        //     * bit3 : fourth octant
    ...
    struct
    {
        //uint16_t    x;              // For future implementation - Horizontal location of the lower left corner of the bar code.
        //uint16_t    y;              // For future implementation - Vertical location of the lower left corner of the bar code.  Readable text will print below this point.
        uint8_t    height;         // Bar code height in dots.
        uint8_t    type;           // Bar code type.  See the USB_PRINTER_POS_BARCODE_FORMAT enumeration.
        uint8_t    textPosition;   // Position of the readable text.  Valid values are BARCODE_TEXT_OMIT, BARCODE_TEXT_ABOVE, BARCODE_TEXT_BELOW, BARCODE_TEXT_ABOVE_AND_BELOW.
        uint8_t    textFont;       // Font of the readable text.  Valid values are dependent on the particular POS printer (BARCODE_TEXT_12x24 and BARCODE_TEXT_18x36 for ESC/POS).
        uint8_t    *data;          // Pointer to the bar code data.
        uint8_t    dataLength;      // Number of uint8_ts of bar code data.
        union
        {
            uint8_t    value;
            struct
            {
                uint8_t    bPrintCheckDigit    : 1;    // Whether or not to print an optional check digit.  Valid for Code39 (USB_PRINTER_POS_BARCODE_FORMAT USB_PRINTER_POS_BARCODE_CODE39) and CODABAR (USB_PRINTER_POS_BARCODE_FORMAT USB_PRINTER_POS_BARCODE_CODABAR) formats only.
            } bits;
        } flags;        
    } sBarCode;
    ...
    struct
    {
        uint16_t    xL;     // X-axis position of the left side of the bevel.
        uint16_t    yT;     // Y-axis position of the top of the bevel.
        uint16_t    xR;     // X-axis position of the right side of the bevel.
        uint16_t    yB;     // Y-axis position of the bottom of the bevel.
        uint16_t    r;      // The radius of the cicle that defines the rounded corner
    } sBevel;
    ...
    struct
    {
        uint16_t    x;      // X-axis position of the center of the circle.
        uint16_t    y;      // Y-axis position of the center of the circle.
    ...
    struct
    {
        uint16_t    fillType;   // The type of fill.  See USB_PRINTER_GRAPHICS_FILL_TYPE for valid values.
        uint16_t    spacing;    // Line spacing for hatched fill (if supported).
        uint16_t    angle;      // Line angle for hatched fill (if supported).
        uint16_t    shading;    // Shading level for shaded fill.  Printer support may be limited.
    } sFillType;

    // This structure is used by the USB_PRINTER_GRAPHICS_LINE command (USB_PRINTER_COMMAND).
    struct
    {
        uint16_t    x1;     // X-axis position of the first point.
        uint16_t    y1;     // Y-axis position of the first point.
        uint16_t    x2;     // X-axis position of the second point.
        uint16_t    y2;     // Y-axis position of the second point.
    } sLine;
    ...
    struct
    {
        uint16_t    startPointHorizontal;   // The horizontal page starting point.
        uint16_t    startPointVertical;     // The vertical page starting point.
        uint16_t    lengthHorizontal;       // The horizontal print length.
        uint16_t    lengthVertical;         // The vertical print length.
        uint8_t    printDirection;         // The print direction and starting point.
    } sPage;

    // This structure is used by the USB_PRINTER_GRAPHICS_POLYGON command (USB_PRINTER_COMMAND).
    struct
    {
        int16_t   numPoints;  // The number of points of the polygon.
        uint16_t    *points;    // The array of polygon points {x1, y1, x2, y2, ... xn, yn}.
    } sPolygon;
    ...
    struct
    {
        uint16_t    xL;     // X-axis position of the left side of the rectangle.
        uint16_t    yT;     // Y-axis position of the top of the rectangle.
        uint16_t    xR;     // X-axis position of the right side of the rectangle.
        uint16_t    yB;     // Y-axis position of the bottom of the rectangle.
    } sRectangle;
...
/****************************************************************************
  Function:
    uint8_t (*USB_PRINTER_LANGUAGE_HANDLER) ( uint8_t address,
        USB_PRINTER_COMMAND command, USB_DATA_POINTER data, uint32_t size, uint8_t flags )
...
typedef uint8_t (*USB_PRINTER_LANGUAGE_HANDLER) ( uint8_t address,
        USB_PRINTER_COMMAND command, USB_DATA_POINTER data, uint32_t size, uint8_t flags );
...
/****************************************************************************
  Function:
    uint16_t USB_PRINTER_EXTERNAL_MEMORY_CALLBACK
        (EXTDATA* memory, LONG offset, uint16_t nCount, void* buffer)
  ...
  Parameters:
    ...
    LONG offset     - uint8_t offset of required data.
    uint16_t nCount     - Number of data uint8_ts to copy into the buffer.
    void *buffer    - Destination data buffer.

  Returns:
    The function returns the number of data uint8_ts transferred.
  ...
  Example:
    ...
    uint16_t ExternalMemoryCallback( EXTDATA *memory, LONG offset, uint16_t nCount, void *buffer )
    {
        int i;
        long address;
...
typedef struct
{
    uint16_t                            vid;            // Printer vendor ID.
    uint16_t                            pid;            // Printer product ID.
    uint16_t                            languageIndex;  // Index into the usbPrinterClientLanguages[] array of USB_PRINTER_INTERFACE structures defined in usb_config.c.
...
/****************************************************************************
  Function:
    uint8_t USBHostPrinterCommandWithReadyWait( uint8_t &returnCode,
                uint8_t deviceAddress, USB_PRINTER_COMMAND command,
                USB_DATA_POINTER data, uint32_t size, uint8_t flags )
...
/****************************************************************************
  ...
  Description:
    This function retrieves the number of uint8_ts copied to user's buffer by
  ...
  Returns:
    Returns the number of uint8_ts most recently received from the Printer
...
/****************************************************************************
  ...
  Parameters:
    deviceAddress   - USB Address of the device
    *status         - pointer to the returned status uint8_t
...
/****************************************************************************
  Function:
    uint32_t USBHostPrinterPosition( uint16_t X, uint16_t Y )
  ...
  Description:
    ...
    (X,Y) coordinate be passed in the (uint32_t) size parameter of the
    USBHostPrinterCommand() function.  This function takes the specified
    coordinate and packs it in the uint32_t as required.
  ...
  Returns:
    uint32_t value that can be used in the USBHostPrinterCommand() function
...
#define USBHostPrinterPosition( X, Y )  (((uint32_t)(X) << 16) | ((uint32_t)(Y) & 0xFFFF))


/****************************************************************************
  Function:
    uint32_t USBHostPrinterPositionRelative( int16_t dX, int16_t dY )
  ...
  Description:
    ...
    change in the (X,Y) coordinates be passed in the (uint32_t) size parameter
    of the USBHostPrinterCommand() function.  This function takes the
    specified coordinate changes and packs them in the uint32_t as required.
...
#define USBHostPrinterPositionRelative( dX, dY )  (((uint32_t)(dX) << 16) | ((uint32_t)(dY) & 0xFFFF))
...
/****************************************************************************
  ...
  Parameters:
    ...
    length         - Number of uint8_ts to be transferred
...
/****************************************************************************
  ...
  Parameters:
    ...
    uint32_t length        - Number of uint8_ts to be transferred
